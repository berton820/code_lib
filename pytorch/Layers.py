...
class YangAttentionPooling(nn.Module):
    def __init__(self, ques_max_len, hidden_size):
        super(YangAttentionPooling, self).__init__()
        self.W = nn.Parameter(torch.empty(1, hidden_size, ques_max_len).uniform_(-1, 1), requires_grad=True).cuda()
        self.Bias = nn.Parameter(torch.empty(1, hidden_size, hidden_size).uniform_(-1, 1), requires_grad=True).cuda()
        self.V = nn.Parameter(torch.empty(1, 1, hidden_size).uniform_(-1, 1), requires_grad=True).cuda()
    ...
    def forward(self, x):
        ...
        """
        # try:
        # todo 暂时先这样但是可能乘积的维度不对
        weight = torch.matmul(self.V, F.tanh(torch.matmul(self.W, x) + self.Bias))
        # print(weight.shape) #print(out.shape)torch.Size([20, 1, 400])
        x_weight = F.softmax(weight, dim=-1) * x  # torch.Size([20, 197, 400])
        out = torch.sum(x_weight, dim=1, keepdim=True)
        return out


class YangPreQueryAttention(nn.Module):
    def __init__(self, query_len, hidden_size):
        super(YangPreQueryAttention, self).__init__()
        # 第一个维度为了boardcast操作定义为1，支持变化的batch
        self.U1 = nn.Parameter(torch.empty(1, query_len, query_len).uniform_(-1, 1), requires_grad=True).cuda()
        self.U2 = nn.Parameter(torch.empty(1, 1, 1).uniform_(-1, 1), requires_grad=True).cuda()
        self.D = nn.Parameter(torch.empty(1, hidden_size, hidden_size).uniform_(-1, 1), requires_grad=True).cuda()

        # todo 看一下relu的维度是不是正确的

    def forward(self, query_pre, query_last):
        assert (query_last.shape[0] == query_pre.shape[0])
        # t = torch.matmul(self.U1, query_pre)
        a = F.relu(torch.matmul(self.U1, query_pre))
        # print(a.shape)  # torch.Size([20, 4, 200])
        # print(self.U2.shape)
        # print(query_last.shape)
        b = F.relu(torch.matmul(query_last, self.U2))
        # print(b.shape)
        return torch.matmul(torch.matmul(a, self.D), b)


class YangGate(nn.Module):
    def __init__(self, ques_max_len, hidden_size):
        super(YangGate, self).__init__()
        self.W = nn.Parameter(torch.empty(1, 1, ques_max_len).uniform_(-1, 1), requires_grad=True).cuda()
        self.Bias = nn.Parameter(torch.empty(1, 1, hidden_size).uniform_(-1, 1), requires_grad=True).cuda()
        self.V = nn.Parameter(torch.empty(1, hidden_size, 1).uniform_(-1, 1), requires_grad=True).cuda()

    def forward(self, x):
        weight = torch.matmul(F.tanh(torch.matmul(self.W, x) + self.Bias), self.V)
        return weight
